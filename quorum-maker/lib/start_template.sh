...
echo "[*] geth --verbosity 6 --datadir qdata" $GLOBAL_ARGS" --rpcport "$R_PORT '--rpccorsdomain "*"' "--port "$W_PORT "#blockMakerPattern# #voterPattern# --minblocktime 2 --maxblocktime 5 --nat extip:"$CURRENT_NODE_IP>> qdata/logs/#nodeName#.log

PRIVATE_CONFIG=#nodeName#.conf geth --verbosity 6 --datadir qdata $GLOBAL_ARGS --rpcport $R_PORT --rpccorsdomain "*" --port $W_PORT #blockMakerPattern# #voterPattern# --minblocktime 2 --maxblocktime 5 --nat extip:$CURRENT_NODE_IP 2>>qdata/logs/#nodeName#.log 
